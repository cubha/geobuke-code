 ...
 */
export function loadPlanSpec(cwd: string): { text: string; source: string; warning?: string } {
  ...
  if (process.env.GBC_SPEC_FILE) {
    ...
    // cwd 밖 공유 명세를 명시 지정하는 정당 용례가 있다. env는 신뢰 경계 밖이 아니므로 막지 않는다.
    // 0.6.1 R6: 경고는 stderr 직출력 대신 반환 필드로 순수화 — 순수 모듈 유일의 스트림 부작용이
    // A-mode in-process 콜백에서 호스트 stderr를 오염시키지 않게. 표면화는 CLI(gbc status)가 담당.
    if (!resolved.startsWith(cwd + sep)) {
      warning = `🐢 gbc: GBC_SPEC_FILE이 프로젝트 밖을 가리킵니다(${resolved}). 의도한 것인지 확인하세요.`;
    }
  ...
  for (const path of candidates) {
    if (existsSync(path)) {
      try {
        ...
        return { text, source: path, warning };
      } catch {
        // 읽기 실패는 다음 후보로
      }
    }
  }
  return { text: "", source: "(없음)", warning };
}
