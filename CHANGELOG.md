...
### Fixed
- **포커스 모드 선택 오염 근본수정** — 0.11.1의 포커스 모드는 사이드바만 숨겼는데, 대화 각 행이 여전히 `++ │ 텍스트 │ ++`(외부 `+` 프레임 좌우 거터 + 대화창 박스 테두리) 구조라 드래그 선택 시 클립보드가 그대로 오염됐다(사용자 실사용 재보고). 포커스 모드에서 외부 프레임·대화창 테두리·타이틀 행 `+` 배경 채움을 전부 걷어내고 타이틀을 mini로 자동 강등한다(Claude Code CLI 방식). 새 레이아웃 계산은 없다 — `computeFrameLayout`의 저해상도 `enabled:false` 경로, `computeResponsiveLayout`의 표시값 오버라이드 등 기존 강등 인프라를 그대로 재사용한다. 실측(tmux `capture-pane`, 120×40): 대화 행 크롬 글리프 **8→0**, 대화 뷰포트 **23→33행**(+10). 사용자의 실제 `titleMode`는 불변이라 Alt+F 해제 시 원래 모드로 복귀한다.
- **최하단 행 `+` 잔상** — 포커스 모드 전환 시 화면 마지막 행(statusline) 뒤에 직전 프레임의 `+` 밴드 꼬리가 남고 재렌더로도 사라지지 않던 결함. ink가 마지막 렌더 행에만 erase-to-EOL을 emit하지 않는 것이 원인으로, 일반 모드에선 마지막 행이 늘 전체 폭이라 드러난 적이 없던 갈래다. 마지막 행이 스스로 전체 폭을 덮게 한다(순수 공백은 ink가 줄 끝에서 잘라내므로 색을 입혀 채운다).
- **Tab 미아 포커스 잔여 갈래** — 0.11.1이 닫은 미아 사이드바 포커스가 **저폭 강등**(사이드바가 폭 부족으로 숨겨진 경우)에서는 그대로 재현됐다. 판정 근거를 `focusMode`에서 실제 렌더 가시성으로 넓혀 근본수정(scope-critic 발견).
- **웰컴카드 Alt+F 키맵 누락** — 0.11.1이 포커스 모드를 도입하며 `?` 도움말에는 넣고 첫 진입 화면인 웰컴카드에는 빠뜨렸다. `?` 도움말은 그것을 눌러본 사람만 보는 순환이라 신규 키가 사실상 미노출이었다. 카드 행 수는 유지한 채 Alt+T와 병기한다(카드 높이가 반응형 강등 사다리의 입력이라 1행만 자라도 저높이 터미널의 강등 임계가 밀린다).

### Changed
- 대화창 박스 크롬(테두리·패딩)이 폭·행수·캐럿 x/y 네 곳에 각각 하드코딩돼 있던 것을 `computeChatBoxChrome` 단일 소스로 모았다. 세그먼트 구분자(`" · "`)도 렌더·문자열 결합·폭 계산 세 곳이 각자 하드코딩하던 것을 `SEGMENT_SEP`으로 단일화했다. `Frame`은 레이아웃을 자체 재계산하지 않고 prop으로 받는다.

검증: `verify.sh --full` 903/903 · scope-critic 5/5 SubTask 통과(1건 실결함 포착·수정) · tmux 실기 캡처로 120×40·80×30 양쪽 확인. hook 계약 무변경 = 재init 불요.
