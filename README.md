...
나중에 `gbc tui`/`gbc run`이 필요해지면 그때 `npm i -g ink react @anthropic-ai/claude-agent-sdk`로 추가하면 된다(미설치 상태로 실행하면 gbc가 설치 명령을 안내한다).
...
```bash
npm i ink react @anthropic-ai/claude-agent-sdk   # optionalDependency — TUI 쓸 때만 설치
gbc tui [--model <모델>]
```
