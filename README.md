...
> **요구사항**: Node.js **>=22**(0.9.0부터, Breaking — `gbc tui`가 요구하는 ink 7/React 19 최소 버전).
...
나중에 `gbc tui`/`gbc run`이 필요해지면 그때 `npm i -g ink@7.1.0 react@19.2.7 @anthropic-ai/claude-agent-sdk@0.3.202`로 추가하면 된다(미설치 상태로 실행하면 gbc가 정확한 버전까지 포함해 설치 명령을 안내한다). **버전을 반드시 명시하라** — 캐럿(`^`)이나 미지정으로 설치하면 사내 프록시 레지스트리 캐시 상태에 따라 다른 버전이 깔려 충돌할 수 있다(아래 [사내 프록시 레지스트리 환경](#사내-프록시레지스트리-nexusartifactory-환경) 참조).
...
| **편집 적용 직후**(0.12.3) | PostToolUse (`Edit\|Write\|MultiEdit`) | 사용자가 승인해 **실제로 적용된** 편집을 작업단위 원장(`.gbc/applied.json`)에 기록 → 이후 판정이 "이미 적용된 편집"을 참고해 같은 형제 케이스를 재차단하지 않는다(아래 [작업단위 구현이력](#작업단위-구현이력-posttooluse-0123)) |
...
### 같은 누락의 반복 차단 억제 (block-repeat, 0.9.3 + 근사매칭 0.13.0)

같은 작업단위(명세 해시) 안에서 같은 침묵-누락 케이스가 편집마다 재차단되면(형제 케이스를 아직 안 다뤘을 뿐인데 매번 같은 사유로 물어봄) 노이즈가 된다. 직전 차단에서 안내된 누락 셋과 (정규화 후) 동일하면 두 번째부터는 사람 승인 없이 조용한 안내로 강등한다(`🐢 같은 누락 케이스가 이미 안내됐습니다`) — `gbc gate review`로 여전히 분류 가능하다.

**0.13.0부터 완전일치 외에 근사매칭도 본다.** LLM이 매 judge 호출마다 같은 잔여 작업을 문구만 다르게(병합·분리·축약) 재서술하면 완전일치 판별이 실패해 재차단이 재발했다(실사용 로그로 확인된 결함). 이제 같은 작업단위 안에서 **안내된 누락 문구를 누적**해두고, 새 누락이 그 누적분을 바이그램 단위로 충분히 재진술하고 있으면(신규 절이 섞이지 않은 재서술) 강등한다. 완전히 새로운 위반(예: 공지된 적 없는 형제 케이스)은 토큰이 일부 겹치더라도 강등되지 않는다. 이전 동작(완전일치만)으로 되돌리려면 `GBC_REPEAT_MATCH=exact`.
...
> **실험적이다(0.7.0 스파이크 → 0.8.0 사후대조 착지).** 기본 제품은 위의 B-모드(Claude Code에 얹는 hook 게이트)이며, A-모드는 그 게이트 루프가 **in-process로 구동되는 에이전트** 위에서도 성립하는지 검증하는 실행 가능한 절개다. 0.8.0부터 A-모드 세션은 `gbc score`로 **통과 후 시나리오 위반율(진짜 M1)**을 사후 채점할 수 있다. 인터페이스·범위는 후속 버전에서 바뀔 수 있다.
...
```bash
# agent-sdk는 optionalDependency — A-모드 쓸 때만 설치. 버전 명시 필수
# (사내 프록시 레지스트리 환경 — [트러블슈팅](#사내-프록시레지스트리-nexusartifactory-환경))
npm i @anthropic-ai/claude-agent-sdk@0.3.202
...
> 0.10.0부터 좌측 상시 사이드바에서 `gbc repos`에 등록된 repo를 opt-in 탭(`Alt+1..9`)으로 전환·병행할 수 있다. 0.10.1부터 대화는 사이드바 옆 **박스형 대화창**에 상주하며(시각행 기준 윈도잉+PgUp/PgDn 스크롤), 0.10.4부터 대화 스크롤백이 **repo별로 격리 보존**된다 — 탭을 오가도 각 repo의 대화가 사라지지 않고, 백그라운드 탭에 도착한 메시지도 그 탭에 쌓여 복귀 시 확인할 수 있다.

`gbc run`(A-모드 CLI 실행)과 같은 in-process 엔진 위에, 승인 프롬프트·실시간 게이트 줄·계측 패널을 갖춘 풀스크린 화면을 얹는다.

```bash
npm i ink@7.1.0 react@19.2.7 @anthropic-ai/claude-agent-sdk@0.3.202   # optionalDependency — TUI 쓸 때만 설치, 버전 명시 필수(아래 참조)
gbc tui [--model <모델>]
```

- **승인 프롬프트(y/n/e/d)** — 게이트가 BLOCK하면 에이전트가 스스로 `gbc spec add "..."`를 도출해 재시도하는 사이클([시나리오 도출 루프](#시나리오-도출-루프-수기-입력-불필요))을, 화면에서 y(승인)/n(거부)/e(수정 후 승인)/d(defer로 보류) 4지선다로 사람이 pause한다. e/d는 그 호출이 실제로 `gbc spec add`를 도출한 경우(spec-add)에만 유효 — 그 외 일반 도구 승인(generic)에선 e/d 모두 n과 동일하게 거부로 처리된다(고무도장 방지 기본값, 화면 라벨에도 반영).
- **Alt+M / Alt+R / Alt+S 토글 패널** — 대화 흐름을 가리지 않고 진짜 M1(위반율)·M2/M3 계측(`Alt+M`), 등록 repo 게이트 상태(`Alt+R`), 설치 스킬 목록(`Alt+S`)을 겹쳐 확인한다. repos 패널은 ↑/↓·Enter로 해당 repo 탭 전환/opt-in까지 된다(0.10.4). Esc로 닫는다.
- **`/` 스킬 드롭다운 · `?` 도움말** — 입력창에서 `/`를 치면 설치 스킬 드롭다운(전방일치 필터·↑/↓·Enter/Tab 완성)이 뜨고, 선택한 스킬은 SKILL.md 본문 주입 방식으로 발동한다(격리 규율상 SDK 자체 스킬 로드는 차단 유지). 입력창이 비어있을 때 `?`는 전체 단축키 도움말 패널을 토글한다(0.10.4).
- **statusline 2줄** — 디렉터리·브랜치·dirty 여부·모델·비용·컨텍스트 토큰 사용량(`12.3k/200k`, 0.11.0부터)을 turn마다 갱신한다.
- **프롬프트 히스토리(↑/↓) 로컬 영속(0.11.0)** — 입력창에서 이전에 보낸 프롬프트를 repo별로 다시 탐색할 수 있다. `~/.gbc/prompt-history.json`에 repo당 최근 100건까지 저장되며(파일 권한 0600, 저장 전 알려진 시크릿 패턴을 best-effort로 마스킹하지만 완전한 시크릿 스캐닝은 아니다 — 민감정보를 직접 타이핑하지 않도록 주의), `GBC_NO_PROMPT_HISTORY=1`로 끄면 저장·조회 둘 다 완전히 비활성화된다(이미 저장된 파일은 직접 삭제해야 한다).
- **격리 규율은 A-모드와 동일** — `gbc tui`를 실행하지 않는 한 ink/react/agent-sdk는 로드되지 않는다(다른 모든 gbc 커맨드·B-모드 hook 핫패스 무영향, 회귀락: `test/tui-isolation.test.mjs`).
- **`!<command>` 셸 직접 실행(0.12.3)** — 입력창에서 `!`로 시작하면 셸 명령을 직접 실행한다(Claude Code CLI `!` 프리픽스와 동일 계열). **사람이 직접 타이핑하는 입력이라 모델 도구 경로가 아니고 게이트 판정을 전혀 경유하지 않는다** — 애초 게이트 범위 밖이라 무충돌. `argv` 배열로 실행하므로(셸 문자열 통짜 실행 아님) 파이프·리다이렉트·`$()` 명령치환·변수확장은 지원하지 않는다 — 따옴표(`'…'`/`"…"`)는 공백 포함 인자를 묶는 용도일 뿐 안에서도 확장되지 않는다. 대화형 명령(`!vim` 등)은 stdin이 없어 즉시 종료되거나 실패한다. 30초 타임아웃·64KB 출력 상한을 넘으면 강제 종료. 스트리밍 응답 중에는 실행할 수 없다(끝난 뒤 다시 시도). 끄려면 `GBC_NO_BANG=1`.

### 사내 프록시(레지스트리 Nexus/Artifactory) 환경

회사 npm 레지스트리가 `registry.npmjs.org`가 아니라 사내 프록시(`npm config get registry`로 확인 가능)면, `gbc tui`/`gbc run` 설치가 아래 순서로 조용히 꼬일 수 있다(실사용 재현 사례):

1. **캐럿(`^`)이나 버전 미지정 설치는 하지 않는다.** `npm i -g ink`처럼 버전을 안 박으면, 사내 프록시가 캐싱한 "latest" 메타데이터가 실제 npmjs.org보다 오래됐을 수 있어(미러 동기화 지연) 의도한 버전과 다른 게 깔린다. **패키지 세 개(`ink`·`react`·`@anthropic-ai/claude-agent-sdk`)는 항상 정확한 버전으로, 한 번의 명령에 같이 설치**한다(위 코드 블록 그대로 복붙 — 개별로 나눠 설치하면 npm이 peer dependency를 따로 해석해 React가 중복 설치될 수 있다):
   ```bash
   npm uninstall -g ink react @anthropic-ai/claude-agent-sdk
   npm install -g ink@7.1.0 react@19.2.7 @anthropic-ai/claude-agent-sdk@0.3.202
   ```
2. **`optionalDependencies`는 실패해도 npm이 조용히 넘어간다(exit 0).** `npm install -g geobuke-code`가 "성공"으로 끝나도 사내 레지스트리가 이 세 패키지(특히 스코프 `@anthropic-ai/*`)를 미러링 안 하거나 차단하면 실제론 하나도 안 깔려있을 수 있다. `gbc tui` 실행 시 뜨는 안내 메시지가 "설치 안 됨"/"버전불일치"/"React 인스턴스 중복" 중 무엇인지에 따라 원인이 다르다(0.9.1부터 진단 메시지가 구체적으로 알려준다 — 안 뜨고 원시 스택트레이스가 그대로 보이면 이슈로 알려달라).
3. **직접 진단**:
   ```bash
   npm ls -g ink react @anthropic-ai/claude-agent-sdk
   ```
...
**[현재 파일 상태] 절단(0.12.1 P3)** — 편집 대상 파일이 8000자를 넘으면 head 고정분(4000자)에 더해 **이번 편집 위치 주변(±1500자)**을 윈도우로 함께 보여준다(이전엔 head-only라 절단면 뒤쪽 정당 기구현 형제가 항상 안 보였다). 전송 총량은 그대로다(8000자 예산 불변, 지연·비용 부담 없음) — 다만 위 "무엇이 외부로 나가는가"의 노출 조건이 "head 안"에서 "head 또는 편집 위치 근처"로 넓어졌다는 뜻이다(security-auditor 지적, 2026-08-10). 공격자가 편집 위치를 직접 통제하긴 어려워 실익은 제한적이지만, 저장소에 쓸 수 있는 주체가 판정에 영향을 줄 수 있다는 구조적 한계 자체는 P2b와 동일하다.

| 지표 | 관측 | B-모드 신뢰도 |
|---|---|---|
| **M2** 게이트 적중 vs 도중발견 | 차단이 잡은 누락 케이스 수 vs `defer add`로 도중 등록된 수 | **강** (defer-registry와 1:1) |
| **M3** 재호출/iteration | 작업단위당 편집 반복 횟수 | proxy |
| **M1** post-gate 재작업 | 통과 후 churn(spec 변경·gate reset·defer) | **약** (churn proxy) — A-모드 세션은 아래 **진짜 M1**로 승격 |

### 진짜 M1 — 사후대조 채점 (0.8.0, A-모드 세션)

**진짜 M1**(통과 후 시나리오 위반율)은 게이트가 "모델이 실제로 무엇을 했나"를 채점하는 **사후 대조**가 필요하다 — B-커널(hook)은 엔진 출력을 못 봐 churn 약신호까지다. A-모드(`gbc run`)는 `.gbc/extraction.jsonl`에 엔진 출력을 남기므로, 이걸 `events.jsonl`과 **session_id로 조인**해 두 지표를 산출한다:

```bash
gbc run "…" --yes      # A-모드 세션 → extraction.jsonl 축적
gbc score              # 사후대조 채점(후보당 모델 1호출) → .gbc/scores.json
gbc metrics            # [진짜 M1] 섹션에 위반율·오탐율 반영
```

- **위반율(LLM 사후대조)** — 통과 당시 명세(`specHash`로 현행 spec.md 또는 `spec.archive`에서 resolve)와 세션의 실제 편집 묶음을 대조해 violated/compliant/unscored 판정. **분모는 채점 완료분만**(unscored를 넣으면 위반율이 과소평가된다), 명세를 resolve 못 하면 다른 명세로 채점하지 않고 unscored(오염 금지). 채점은 비용이 드는 **명시 명령**(`gbc score`)이다 — 게이트 핫패스·metrics 순수 집계에 절대 실리지 않는다.
- **오탐율(행동신호)** — 차단의 정당성을 LLM 재판정이 아니라 **차단 이후 실제로 일어난 일**로 분류한다(게이트 자체가 LLM이라 재판정은 일치도지 truth가 아니다): `block→spec 보강→통과`=정상작동 / `block→편집 수정만으로 통과`=자가수정(모호) / `block→gate-reset·bypass`=무시, `block→포기`=오탐 후보 / `block→fail-open 통과`=판정불능(분자·분모 모두 제외). events만으로 산출돼 B-모드 차단에도 적용된다.
- **정직 규율** — 표본 0이면 rate를 0%가 아니라 `—(표본 0)`으로 표시하고, extraction 없는 B-모드 세션은 `채점가능` 분모에서 정직하게 제외된다. 통계적 유의미성은 코퍼스 축적(도그푸딩)에 달렸다 — 지금 숫자는 기전 실증이지 대표 통계가 아니다.
...
- `M1 churn` → **약신호 proxy** — "통과 후 결함 수"로 과대해석 금지. 진짜 M1은 A-모드 세션 한정으로 측정된다(`gbc score` 사후대조, 0.8.0) — B-모드 churn은 여전히 proxy다
...
- 제품 범위 = **B-커널**(CC-native hook + defer-registry + /gate) + 그 위 운영층(누락 케이스 일괄 분류·scope 사후 판정·크로스-repo 가시성/건강성·판정 드리프트 회귀락·관측 계측 M1~M3) + **사후 결과검증(`gbc verify`)**. **standalone TUI**는 후속 A(public) 영역이다. **A-모드 엔진 래핑은 0.7.0 스파이크(`gbc run`)로 절개됐고, 0.8.0이 그 위에 진짜 M1 사후대조(`gbc score`)를 착지시켰다** — 단, 이것은 **루프 자동 채점이 아니라 명시 호출**이고(세션이 끝난 뒤 사람이 부른다), A-모드 세션(extraction 존재)만 채점할 수 있으며, 지금 숫자는 기전 실증 단계라 통계적 대표성이 없다. `gbc verify`는 그 사전 단계로, 명시 호출 시 케이스↔증거를 사다리로 대조한다.
