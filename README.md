...
- 제품 범위 = **B-커널**(CC-native hook + defer-registry + /gate) + 그 위 운영층(누락 케이스 일괄 분류·scope 사후 판정·크로스-repo 가시성/건강성·판정 드리프트 회귀락·관측 계측 M1~M3) + **사후 결과검증(`gbc verify`)**. **standalone TUI·엔진 래핑 추출 모드·진짜 자동 사후대조 M1**(통과 후 시나리오 위반율을 루프에서 채점)은 후속 A(public) 영역이다 — `gbc verify`는 그 사전 단계로, 명시 호출 시 케이스↔증거를 사다리로 대조한다(루프에 박힌 자동 채점은 아직 아니다).
