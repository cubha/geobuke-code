...
이때 **`.gbc`가 하나가 아니라 여러 겹**일 수 있다 — 과거 버전(0.12.1 이전)에서 하위 디렉토리를 한 번이라도 읽으면 그 자리에 빈 `.gbc`가 생기던 결함(`gbcDir()`의 mkdir-on-read)이 있었다. 이 화석이 하위에 남아 있으면, 조상 walk-up이 **진짜 루트보다 먼저 화석을 만나 거기서 멈춰버려** 진짜 루트의 `spec.md`·`.claude/settings.json`을 영원히 못 보게 된다(모든 판정이 "명세 소스: (없음)"으로 조용히 오판됨).
