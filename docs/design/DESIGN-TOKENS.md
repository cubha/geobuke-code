...
canonical 디자인 토큰. 소스 오브 트루스는 코드(`src/tui/format.ts`의 `PALETTE`/`MASCOT_S2`/`MASCOT_C4`, `src/tui/ui/theme.ts`의 `toneColor`) — 이 문서는 그 값을 사람이 읽기 쉬운 형태로 미러링한다. 값이 갈리면 코드가 이긴다.
...
- **S2 "워커 v2"** — 측면 보행 자세, 24×10px → 5줄. 기본(≥60열). 구 C1의 개선판(플레이트 패턴·꼬리·입으로 표정 부여, 치수 동일). 마스코트 개편 4종 시안 중 사용자 최종확정(2026-07-13), 변경 시 이 문서와 `format.ts`의 `MASCOT_S2` 동시 갱신.
