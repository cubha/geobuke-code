# DESIGN-TOKENS — gbc tui (0.9.0 A3a, 0.9.3 D1/D2 + 마스코트·배지 최종판, 0.10.1 레이아웃 재확정 갱신)

canonical 디자인 토큰. 소스 오브 트루스는 코드(`src/tui/format.ts`의 `PALETTE`/`MASCOT_S2`/`MASCOT_C4`/`WORDMARK_GEOBUKE`/`SHELL_BADGE_GLYPH`/`renderWordmark`/`renderShellBadge`/`formatWelcomeCard`/`selectHeroLayout`/`computeFrameLayout`, `src/tui/ui/theme.ts`의 `toneColor`) — 이 문서는 그 값을 사람이 읽기 쉬운 형태로 미러링한다. 값이 갈리면 코드가 이긴다.

시각 기준(Ground Truth) 원본: 시안 A(토글 패널)+statusline 2줄+그린 톤, 사용자 확정 2026-07-10(`project_0_9_0_tui_stack_decision.md`). 스플래시(워드마크+마스코트+안내카드) 레이아웃 확정: 아티팩트 `cb7c6b1c-f254-415e-991a-a43d2a2e1f33`, 사용자 확정 2026-07-14. 마스코트 카와이 재설계·등껍질 배지는 실 터미널 시안 반복(tmux 실측)으로 사용자 확정 2026-07-14~15(`project_0_9_3_gate_false_positive_plan.md` 4~12차 후속). **0.10.1 레이아웃 재확정**: 카드+사이드바 동일폭 34·워드마크 상시노출·외부 '+' 프레임 — 아티팩트 `ff0eb0b1-192f-4334-b8d3-761deeae23c0`, braintrust 4렌즈 적대검토 통과(2026-07-20, `project_tui_turn_latency_conversation_ux.md`).
...
- **S3 "카와이"**(상수명은 `MASCOT_S2` 유지) — 측면 보행 자세, 30×16px → 8줄. 스플래시 히어로 기본(≥60열, `selectMascot`). **0.10.1부터 좌측 상시 사이드바도 이 마스코트를 직접 지정**(사이드바 내부폭 30에 무잘림으로 들어감 — 이전엔 `selectMascot(SIDEBAR_COLUMNS)` 오용으로 항상 C4 미니만 뜨던 결함). 원/타원 래스터화+실루엣 전체 윤곽선(`B`) 기법 — 큰 머리가 목으로 몸통 밖까지 이어지고 눈 하이라이트(`W`)·볼터치(`P`)로 표정을 준다. 눈은 46×26 승인 시안과 동일한 **절대 반경(1.6×1.7)** 고정(비례축소 금지 — 저해상도에서 뭉개짐). 사용자 최종확정 2026-07-14, **이후 변경 금지(사용자 픽스 선언)**.
- **C4 미니** — 12×6px → 3줄. 스플래시 히어로 `<60`열 폴백(`selectMascot`, 임계값은 `SPLASH_WIDE_MIN_COLUMNS`). 사이드바에는 더 이상 쓰이지 않는다(위 참조).
...
conhost급(ansi16) 폴백은 상위 3행=밝은 그린(ANSI 92), 하위 3행=그린(ANSI 32) 2단 근사. 워드마크 아래 태그라인(`formatTagline`): `거북이코드 v{package.json 버전} · 계획↔구현↔검증 게이트`, 색 `#166534`(PALETTE `D`와 동일), 워드마크 우측 정렬. **0.10.1**: 워드마크 표시 여부가 2컬럼 병치와 독립 판정으로 분리됐다(`selectHeroLayout`) — 72열 이상이면 마스코트/카드가 세로 스택이어도 워드마크는 항상 상단 전체폭에 뜬다. `SPLASH_WORDMARK_MIN_COLUMNS`(72열) 미만이면 워드마크 대신 태그라인 텍스트만 표시(판독 불가한 아스키 잔해 방지 강등 규칙).

## 안내카드 (0.9.3 D1 — `formatWelcomeCard`, 0.10.1 폭·카피 재확정)

3섹션 구성, Ink `Box borderStyle="round" borderColor="green"` 고정폭 **34칸**(`WelcomeCard.tsx` `CARD_WIDTH` — 0.10.1부터 좌측 사이드바와 **동일폭**, 이전 54. 내부폭 30 = 34−테두리2−paddingX2):
...
## 레이아웃 — 시안 A(토글 패널형) + 스플래시 히어로(0.9.3 D2, 0.10.1 임계값 재계산)
...
- **좌측 상시 사이드바**(`Sidebar.tsx`, 0.10.0 A3b 다중탭 스위처) — 고정폭 **34**(`SIDEBAR_COLUMNS`, 0.10.1부터 안내카드와 동일폭 통일. S2 카와이 마스코트 30폭 무잘림 최소치+테두리2+paddingX2). 등록 repo 목록(opt-in 탭 상태 글리프 ▶⏸●○✖)+하단 마스코트(S2).
- **스플래시 히어로**(`SplashHero.tsx`) — 폭 3단 반응형, 임계값 3개가 분리돼 있다: 마스코트용(`SPLASH_WIDE_MIN_COLUMNS`=60) · 워드마크 상시노출용(`SPLASH_WORDMARK_MIN_COLUMNS`=72, 0.10.1 신설 — 워드마크59+`WORDMARK_BADGE_GAP`4+배지9) · 2컬럼 병치용(`SPLASH_HERO_MIN_COLUMNS`=73, 0.10.1 재계산 — `HERO_LEFT_MARGIN`3+마스코트30+`MASCOT_CARD_GAP`6+`CARD_WIDTH`34. 이전 96은 카드폭54 기준이라 카드34 통일 후 더 이상 유효하지 않음). `selectHeroLayout(columns)`가 `showWordmark`/`twoColumn` 두 독립 불리언으로 판정:
  - `<60`열: 마스코트 C4 미니 + 안내카드 세로 스택(워드마크 생략)
  - `60~71`열: 마스코트 S2 + 안내카드 세로 스택(워드마크 생략)
  - `72`열(정확히): 워드마크+배지+태그라인 노출 + 마스코트/카드는 아직 세로 스택(좁은 중간 상태)
  - `≥73`열: 워드마크(그라데이션)+등껍질 배지+태그라인 → 마스코트 S2 + 안내카드 **2컬럼 병치**(Ink `alignItems="center"`로 마스코트를 카드 세로 중앙에 정렬).
  ...
  - **여백**(Ink `margin*` props, `SplashHero.tsx`): 히어로 위 2줄(`HERO_TOP_MARGIN`, 태그라인↔마스코트 여백과 대칭) · 좌측 여백 3칸(`HERO_LEFT_MARGIN`, 전체 히어로) · 워드마크↔배지 4칸(`WORDMARK_BADGE_GAP`) · 마스코트↔카드 6칸(`MASCOT_CARD_GAP`) · 태그라인↔마스코트+카드 행 2줄(`marginTop={2}`, 워드마크 노출 시) · 카드↔웰컴 1줄(`marginTop={1}`) · 웰컴↔입력창 2줄(`marginBottom={2}`, Static 1회 커밋이라 세션 내내 재적용 안 되고 스플래시 직후에만 남음).
...
활성 시 스플래시 히어로·ReposPanel의 가용폭은 `columns`가 아니라 `computeFrameLayout(columns, rows).innerColumns`(= `columns − 거터×2`)를 써야 한다. 스트리밍 프리뷰·승인박스 행 예산(`computePreviewRowBudget`)도 `PREVIEW_RESERVED_ROWS + bandRows×2`로 밴드 2행을 추가 예약한다.
