...
// ── 반응형 임계값 분리 (0.9.3 D1, 0.10.1 braintrust 재확정) — 마스코트 폴백(60열)·워드마크
// 상시노출(72열)·2컬럼 병치(73열)는 서로 다른 레이아웃 결정이라 단일 임계값을 공유하면 안
// 된다. 0.10.1부터 워드마크는 "96열 이상만" 분기가 사라지고 SPLASH_WORDMARK_MIN_COLUMNS(72)만
// 넘으면 항상 상단 전체폭에 뜬다(marginLeft 0 전제 — 워드마크59+WORDMARK_BADGE_GAP4+배지9=72).
// SPLASH_HERO_MIN_COLUMNS(2컬럼 병치)는 카드 폭 통일(54→34)로 93→73 재계산
// (HERO_LEFT_MARGIN3+마스코트30+MASCOT_CARD_GAP6+CARD_WIDTH34=73). 72<73이라 [72,73) 구간은
// "워드마크는 뜨지만 마스코트/카드는 아직 세로스택"인 좁은 중간 상태 — selectHeroLayout이 이
// 두 판정을 독립 불리언으로 반환한다.
test("SPLASH_WIDE_MIN_COLUMNS(마스코트)·SPLASH_WORDMARK_MIN_COLUMNS(워드마크)·SPLASH_HERO_MIN_COLUMNS(2컬럼 병치)는 별개 값", () => {
  assert.equal(SPLASH_WIDE_MIN_COLUMNS, 60);
  assert.equal(SPLASH_WORDMARK_MIN_COLUMNS, 72);
  assert.equal(SPLASH_HERO_MIN_COLUMNS, 73);
  assert.ok(SPLASH_HERO_MIN_COLUMNS > SPLASH_WORDMARK_MIN_COLUMNS);
  assert.ok(SPLASH_WORDMARK_MIN_COLUMNS > SPLASH_WIDE_MIN_COLUMNS);
});

test("selectHeroLayout: 72열 미만 — 워드마크 생략(태그라인만), 마스코트/카드 세로스택", () => {
  const layout = selectHeroLayout(71);
  assert.equal(layout.showWordmark, false);
  assert.equal(layout.twoColumn, false);
});

test("selectHeroLayout: 정확히 72열 — 워드마크 상시노출 시작, 아직 세로스택([72,73) 중간 상태)", () => {
  const layout = selectHeroLayout(72);
  assert.equal(layout.showWordmark, true);
  assert.equal(layout.twoColumn, false);
});

test("selectHeroLayout: 정확히 73열 — 워드마크+2컬럼 병치 둘 다 활성", () => {
  const layout = selectHeroLayout(73);
  assert.equal(layout.showWordmark, true);
  assert.equal(layout.twoColumn, true);
});

test("selectHeroLayout: 넉넉한 폭(120열) — 둘 다 활성 유지", () => {
  const layout = selectHeroLayout(120);
  assert.equal(layout.showWordmark, true);
  assert.equal(layout.twoColumn, true);
});

test("selectHeroLayout: 순수성 — 같은 입력엔 항상 같은 출력", () => {
  assert.deepEqual(selectHeroLayout(80), selectHeroLayout(80));
});
...
test("computeContentColumns: 2컬럼 레이아웃(사이드바 포함) 100열 터미널이 히어로 임계값(73) 밑으로 떨어질 수 있음을 실증", () => {
  // 이게 ST9의 핵심 동기 — 전체 100열은 SPLASH_HERO_MIN_COLUMNS(73, 0.10.1 카드34 재계산값)를
  // 넘지만, 사이드바(34)를 뺀 가용폭(66)은 73 밑으로 떨어진다. 이 값을 SplashHero columns
  // prop에 넣지 않으면 실제로 안 맞는 2컬럼 병치 레이아웃을 잘못 선택한다(터미널 폭 재산정
  // 없이 columns를 그대로 쓰던 기존 버그, braintrust R1 지적). 0.10.0 당시 128열/96 조합은
  // 카드34 재계산 후 94≥73이 되어 더 이상 이 실증에 못 쓴다 — 100열/66으로 교체.
  const available = computeContentColumns(100, SIDEBAR_COLUMNS);
