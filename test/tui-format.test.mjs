...
test("MASCOT_S2: 모든 행 길이가 동일하고, '.' 아니면 PALETTE에 정의된 키만 사용", () => {
  const width = MASCOT_S2[0].length;
  for (const row of MASCOT_S2) {
...
// ── 워드마크 노출 임계값 (0.9.3 D1, 0.10.1 braintrust 재확정 → SubTask8 추가 단순화) — 마스코트
// 폴백(60열)·2컬럼 병치(73열) 개념은 마스코트+카드가 스플래시 히어로에서 빠지며(SubTask9/10, 카드는
// 상시 좌측 스택으로 이동·독립 마스코트는 사이드바 것 하나로 통합) 폐기됐다. 이제 히어로가 판정할
// 건 "워드마크를 상단 전체폭에 그릴지"뿐 — shouldShowWordmark 단일 불리언 함수로 충분하다.
test("SPLASH_WORDMARK_MIN_COLUMNS: 72 (워드마크59+WORDMARK_BADGE_GAP4+배지9)", () => {
  assert.equal(SPLASH_WORDMARK_MIN_COLUMNS, 72);
});

test("shouldShowWordmark: 72열 미만은 false(태그라인 텍스트로 강등)", () => {
  assert.equal(shouldShowWordmark(71), false);
});

test("shouldShowWordmark: 정확히 72열부터 true", () => {
  assert.equal(shouldShowWordmark(72), true);
  assert.equal(shouldShowWordmark(120), true);
});
...
// ── 2컬럼 레이아웃 가용폭 재산정 (0.10.0 A3b ST9) — 대화 컬럼은 전체 터미널 폭이 아니라 "좌측
// 사이드바를 뺀 나머지"만 쓸 수 있다. 임계값(SPLASH_WORDMARK_MIN_COLUMNS)은 무변경 — 그 임계값에
// 넣는 *입력값*을 전체 폭에서 사이드바 폭을 뺀 값으로 바꾸는 게 이 함수. ──
...
test("SIDEBAR_COLUMNS: WelcomeCard CARD_WIDTH와 동일폭 34(braintrust 2026-07-20 통일 결정)", () => {
  ...
  assert.ok(SIDEBAR_COLUMNS < SPLASH_WORDMARK_MIN_COLUMNS, "사이드바가 워드마크 임계값보다 넓으면 좁은 터미널에서 남는 폭이 없다");
});
...
test("computeContentColumns: 사이드바 포함 100열 터미널이 워드마크 임계값(72) 밑으로 떨어질 수 있음을 실증", () => {
  // ST9의 핵심 동기 — 전체 100열은 SPLASH_WORDMARK_MIN_COLUMNS(72)를 넘지만, 사이드바(34)를
  // 뺀 가용폭(66)은 72 밑으로 떨어진다. 이 값을 shouldShowWordmark에 넣지 않으면 실제로 안
  // 맞는 워드마크 노출 판정을 잘못 내린다(터미널 폭 재산정 없이 columns를 그대로 쓰던 기존
  // 버그, braintrust R1 지적).
  const available = computeContentColumns(100, SIDEBAR_COLUMNS);
  assert.ok(available < SPLASH_WORDMARK_MIN_COLUMNS, `사이드바 반영 후 가용폭(${available})이 임계값보다 작아야 이 테스트가 의미 있음`);
});
