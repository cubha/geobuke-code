...
// ── [현재 파일 상태] (0.9.3 ST3) — diff만으로는 안 보이는 기구현 형제 케이스를 판별하게 함.
// fa-support 도그푸딩 리포트: import 한 줄 추가 편집이 "명세 소스 없음"으로 오판되거나, 파일에
// 이미 구현된 케이스를 diff에 안 보인다고 침묵 누락으로 재차단하던 것의 근본원인.

test("buildUserMessage: currentFileContent 제공 시 [현재 파일 상태] 블록에 원문 그대로 포함 (ST3)", () => {
  const m = buildUserMessage("plan", "edit", [], [], "function login() { /* 이미 구현됨 */ }");
  assert.match(m, /\[현재 파일 상태\]/);
  assert.match(m, /function login\(\) \{ \/\* 이미 구현됨 \*\/ \}/);
});

test("buildUserMessage: currentFileContent 생략/빈 문자열이면 [현재 파일 상태] 섹션 자체를 생략(ST3 골든replay 실측 — placeholder가 모델 오염 유발해 섹션 통째로 뺌)", () => {
  const withoutArg = buildUserMessage("plan", "edit", [], []);
  assert.doesNotMatch(withoutArg, /\[현재 파일 상태\]/);
  const withEmpty = buildUserMessage("plan", "edit", [], [], "");
  assert.doesNotMatch(withEmpty, /\[현재 파일 상태\]/);
  const withWhitespace = buildUserMessage("plan", "edit", [], [], "   \n  ");
  assert.doesNotMatch(withWhitespace, /\[현재 파일 상태\]/, "공백만 있는 내용도 미제공으로 취급");
});

test("buildUserMessage: [현재 파일 상태]는 [현재 편집] 앞에 위치(파일의 '이전' 상태 → '지금 바뀌는 것' 순서) (ST3)", () => {
  const m = buildUserMessage("plan", "edit-marker-xyz", [], [], "file-content-marker-abc");
  const fileIdx = m.indexOf("file-content-marker-abc");
  const editIdx = m.indexOf("edit-marker-xyz");
  assert.notEqual(fileIdx, -1, "파일상태 마커가 실제로 본문에 있어야 함(트리비얼 -1 통과 방지)");
  assert.notEqual(editIdx, -1);
  assert.ok(fileIdx < editIdx);
});
...
// "diff 밖에 이미 구현돼 있으면 missing 제외" 규칙을 거꾸로 적용하면 안 된다 — 구버전에만 있던
// 형제가 새 전체 내용에서 빠지면 그건 "이미 구현됨"이 아니라 덮어쓰기로 인한 회귀다.
test("GATE_SYSTEM: 전체 작성/덮어쓰기(Write)는 구버전 형제가 새 내용에 없으면 여전히 missing이라는 규칙 포함 (ST3 확대수정)", () => {
  assert.match(GATE_SYSTEM, /전체 작성\/덮어쓰기/);
  assert.match(GATE_SYSTEM, /회귀/);
});
...
// 0.9.3 ST4 — ackDefer: "이미 완료된 항목" 직접 등록(open→resolved 생애주기를 거치지 않는다).
// addDefer(open으로 시작)와 달리, 애초에 defer한 적 없는 케이스를 "이미 구현돼 있었다"로 바로
// 기록한다 — resolvedDeferItems()가 이를 읽어 judge의 [이미 완료된 항목]에 실어 재차단을 막는다.
test("ackDefer: status=resolved로 직접 생성(open 단계 거치지 않음), resolvedDeferItems에 즉시 반영, origin=ack 감사표시(scope-critic 확대수정)", () => {
  const dir = mkdtempSync(join(tmpdir(), "gbc-ack-"));
  try {
    const r = ackDefer(dir, "이미구현된케이스");
    assert.equal(r.added, true);
    assert.equal(r.entry.status, "resolved");
    assert.equal(r.entry.origin, "ack", "ack로 생성된 엔트리는 사후 감사 가능하게 출처 표시");
    assert.deepEqual(resolvedDeferItems(dir), ["이미구현된케이스"]);
    assert.deepEqual(activeDeferItems(dir), [], "ack는 미해결 defer 목록엔 안 나타남(처음부터 완료)");
    // 저장→재로드에도 origin 보존(promote 마이그레이션 경로 확인)
    assert.equal(loadDefers(dir)[0].origin, "ack");
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test("addDefer→resolveDefer(정상 생애주기)는 origin 없음 — ack와 구분되는 것이 감사 가치의 핵심", () => {
  const dir = mkdtempSync(join(tmpdir(), "gbc-genuine-resolve-"));
  try {
    addDefer(dir, "진짜미룬케이스");
    resolveDefer(dir, "1");
    const entry = loadDefers(dir)[0];
    assert.equal(entry.status, "resolved");
    assert.equal(entry.origin, undefined, "정상 defer→resolve 경로는 origin 미지정(ack와 구분)");
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test("ackDefer: 이미 resolved인 동일 텍스트 재등록은 skip(중복 방지, addDefer의 미해결 dedup과 대칭)", () => {
  const dir = mkdtempSync(join(tmpdir(), "gbc-ack-dup-"));
  try {
    const r1 = ackDefer(dir, "케이스X");
    assert.equal(r1.added, true);
    const r2 = ackDefer(dir, " 케이스X "); // 정규화 동일
    assert.equal(r2.added, false);
    assert.equal(loadDefers(dir).length, 1);
  } finally {
    rmSync(dir, { recursive: true, force: true });
...
test("resolveRefs: spec/defer 분류 + spec 우선 dedup", () => {
  ...
  // 한쪽만
  assert.deepEqual(resolveRefs(M, "all", ""), { toSpec: M, toDefer: [], toAck: [] });
  assert.deepEqual(resolveRefs(M, "", "all"), { toSpec: [], toDefer: M, toAck: [] });
});

// 0.9.3 ST4 — 3분류(ack): 게이트가 "누락"으로 잘못 도출했지만 실제론 이미 구현된 케이스를
// [이미 완료된 항목] 채널(judge의 resolved 파라미터)로 즉시 편입시키는 경량 통과 채널.
// fa-support 리포트 요청: "오탐 시 에이전트가 '이미 등록됨/후속 예정'을 기계가독으로 응답해
// 게이트를 통과시키는 경량 채널이 있으면 해명 오버헤드가 줄어듦".
test("resolveRefs: ack 3순위 분류(spec > defer > ack 우선순위 — 셋 다 걸리면 spec만)", () => {
  assert.deepEqual(resolveRefs(M, "", "", "1 2 3"), {
    toSpec: [],
    toDefer: [],
    toAck: M,
  });
  // spec·defer·ack 전부 1을 참조 — spec이 이긴다(defer 우선순위와 동일 원칙 확장)
  assert.deepEqual(resolveRefs(M, "1", "1", "1"), {
    toSpec: ["중복 이메일"],
    toDefer: [],
    toAck: [],
  });
  // defer·ack만 겹침 — defer가 ack보다 우선(명시적 '미룸'이 '이미완료' 주장보다 사람의 확정적 의도)
  assert.deepEqual(resolveRefs(M, "", "1", "1"), {
    toSpec: [],
    toDefer: ["중복 이메일"],
    toAck: [],
  });
  // ackRefs 생략 시 하위호환(빈 toAck)
  assert.deepEqual(resolveRefs(M, "1", "2"), {
    toSpec: ["중복 이메일"],
    toDefer: ["비밀번호 길이"],
    toAck: [],
  });
...
// ===== 0.5.4 ST-P0-2: 자기파일 제외 — production형 경로 조합(절대 file_path × ./상대 grep) =====
// 기존 테스트는 양쪽 다 상대경로 픽스처라 endsWith 우연 일치로 버그가 은폐됐다(분석 2026-07-02).

test("collectGrepContext: 절대 file_path × './'상대 grep 출력에서도 자기 파일 매치를 제외한다", async () => {
  const cwd = "/mnt/proj";
  // production 실제 조합: CC file_path=절대경로, grep -rn ... . 출력=./상대경로
  const entries = [scopeEntry({ file: "/mnt/proj/src/a.ts", edit: "export function fooBarBaz() {}" })];
  const grep = async () => "./src/a.ts:3: export function fooBarBaz() {}\n./src/other.ts:9: fooBarBaz();";
  const { context, filesWithContext } = await collectGrepContext(cwd, entries, { grep });
  assert.ok(!context.includes("./src/a.ts:3"), "자기 파일 정의 라인은 컨텍스트에서 제외");
  assert.ok(context.includes("./src/other.ts:9"), "타 파일 호출부는 유지");
  assert.ok(filesWithContext.has("/mnt/proj/src/a.ts"), "타 파일 매치가 있으므로 등록");
});

test("collectGrepContext: 자기 파일 매치뿐이면 컨텍스트 없음(하드가드 신호 유지)", async () => {
  const cwd = "/mnt/proj";
  const entries = [scopeEntry({ file: "/mnt/proj/src/a.ts", edit: "export function fooBarBaz() {}" })];
  const grep = async () => "./src/a.ts:3: export function fooBarBaz() {}";
  const { context, filesWithContext } = await collectGrepContext(cwd, entries, { grep });
  assert.equal(context, "", "자기참조만이면 빈 컨텍스트");
  assert.equal(filesWithContext.size, 0, "filesWithContext 미등록 → 미평가(degraded) 경로");
});
