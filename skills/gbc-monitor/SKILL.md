...
- **`[P2b] 근거주입 재판정`(0.12.0)** → 차단 판정을 grep 실측 근거로 **재검토**한 결과다. **`근거주입 0건`은 결함이 아니다** — 매치가 하나도 없으면 재판정 자체를 생략하므로, 차단이 드물거나 케이스 심볼이 코드에 없으면 자연히 0이다. 볼 것은 두 가지: ⓐ **전환율**이 P2b가 오탐을 실제로 해소한 유일한 정량 신호(`—(표본 0)`은 0%가 아니라 "아직 안 돌았다"), ⓑ **⚠️ 근거가 온전히 실리지 못한 판정** 라인이 뜨면 그만큼 전환율이 **과소평가**된 것이다. **`파일절단 N/M건`**은 P2b가 존재해야 하는 이유의 크기 — 8000자 절단면 뒤에 이미 구현된 형제 케이스가 1차 judge에 안 보인다는 뜻이고, 비율이 높을수록 근거주입 없이는 오탐이 나기 쉬운 repo다.
